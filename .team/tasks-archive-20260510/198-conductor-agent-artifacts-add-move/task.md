...
- **Agent は `{{OUTPUT_DIR}}` にだけ書く**（project-level `artifacts/` も `.team/artifacts/` も Agent は触らない）
...
- **調査系タスクの判定条件を明記**:
  ...
  - または、`{{OUTPUT_DIR}}` に summary.md 以外のレポート系ファイルが存在する
- **登録ルール**:
  - `{{OUTPUT_DIR}}` にレポートファイル（例: `research.md`, `report.md` 等の命名）があればそれを最優先で登録
  - なければ summary.md を登録
...
- 次の警告ボックスを追加:
  ...
  > - タスク本文に `artifacts/foo.md` 等のリテラルパスが書かれていても、それは慣習的な指示であり、実際には `{{OUTPUT_DIR}}/foo.md` に書くこと
