...
| `clear-conductor` | `broken` 状態の Conductor を Manager の Conductor プールから外す手動回復経路。**Conductor 自身の surface は閉じない** — `CONDUCTOR_CLEAR` メッセージ → daemon が watcher 2 連停止 + 自 Agent surface close（所有判定は `conductor.agents` に限定。pane 同居の無関係 surface には触れない）+ `state.conductors.delete` + `maxConductors -1`（topup の穴埋め防止）を行う。クリア後の surface は `elevens spawn-conductor` で通常経路の再登録により再利用できる（旧仕様の `clearedConductorSurfaces` による再 self-register 恒久拒否は、silent 拒否 → SESSION_STARTED master fallback による Master 誤登録事故を生んだため撤去）。surface 実在性は分岐に使わず観測ログ用にのみ取得し、`conductor_pruned reason=user_clear`（surface 不在時 `user_clear_surface_missing`）を `manager.log` に記録する。`maxConductors` は daemon 再起動 / `cmux-team start` で config 値に戻る。`--surface` 省略時は `CMUX_SURFACE` → `cmux identify` で自動解決。`broken` 以外は error 終了し `abort-task` / `restart-task` / `reset-conductor` を誘導する（C-I3 不変条件の解除手段は本コマンドと `reset-conductor` のみ） |
...
- `dashboard.port` — dashboard server の固定 listen port（T034）。整数 `[1, 65535]`、未指定 / `0` は ephemeral（従来どおり）。不正値（非整数 / 範囲外 / 型違反）は default（ephemeral）に倒し `dashboard_port_config_invalid` を warn log する。指定 port の bind 失敗時は ephemeral へフォールバックせず dashboard 起動をスキップ（daemon は継続、`dashboard_server_start_failed` ログ）。詳細は [`12-web-dashboard.md`](12-web-dashboard.md) §2.3
- `dashboard.lanAccess` — dashboard server を LAN に公開するか（default `false`）。`true` のとき `0.0.0.0` に bind し、`team.json` の `dashboardServer.lanUrl`（`http://<LAN-IP>:<port>`）を書き出して TUI Settings タブに `/files` の QR コードを表示する。`false`（既定）は `127.0.0.1` のみに bind。厳密に `true` のときだけ有効化し、非 boolean / undefined は `false` に倒す（fail-safe）。**⚠️ 認証は無い** — 有効化すると到達可能なネットワーク上の誰でも `docs/` / `.team/artifacts/` / `.team/output/` と metrics を閲覧できる。信頼できるネットワークでのみ使うこと。詳細は [`12-web-dashboard.md`](12-web-dashboard.md) §2.3.1
