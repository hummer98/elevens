...
- **`/files` ビューワーの左ペインにヘッダ（ソート + タイプフィルター）を追加**: 固定ヘッダで `name` / `mtime` / `size` のソート（既定は**更新日時の降順**、クリックで昇順・降順をトグル）と `md` / `html` / `画像` の表示 On/Off フィルターを操作できる。ソートは既存ノードの並べ替え、フィルターは CSS のみで適用するためツリーの展開状態を壊さない。設定はブラウザに保存される。詳細は `docs/spec/12-web-dashboard.md` §8.6
- **`elevens open <file>` 追従モードを追加**: `docs/` / `.team/artifacts/` / `.team/output/` 配下のファイルを指定すると、開いている `/files` ビューワーのタブが**新規タブを増やさずに ~1 秒以内で該当ファイルへ自動で切り替わる**。CLI が `.team/files-focus.json` を書き、SPA が `GET /files/_focus` をポーリングして移動する。`--no-launch` でブラウザ起動を抑止可能。詳細は §8.7
- **Issues タブの同期中にスピナーアニメーションを表示** (T039): gh sync の初回同期 / 再同期中に回転スピナーを表示し、同期が進行中であることが分かるようにした

### Fixed

- **`/files` ビューワーで白背景前提の HTML が判読不能になる不具合を修正**: コンテンツ表示用 iframe をダーク背景にしていたため、背景色を明示しない white-on-black 想定の task report HTML の黒文字がダーク下地に透けて読めなくなっていた。iframe をブラウザ既定の白背景に戻して任意の HTML を「白い紙」の上に忠実表示するようにし、ビューワー自身が生成する Markdown / ディレクトリ一覧ページは引き続きダークテーマを維持する

## [0.15.0] - 2026-06-15

### Changed

- **dashboard ファイルビューワー (`/files`) を 2 ペイン + ダークテーマに刷新** (T038): 従来はディレクトリ index と単一ファイル表示が別ページの 1 ペイン構成だったのを、左サイドバーにファイル/ディレクトリツリー（展開・折りたたみ）、右ペインに選択ファイルの内容を表示する一般的な 2 ペインレイアウトに作り変えた。ダークテーマを採用（既存 dashboard SPA のトーンに統一）。ファイルの更新時刻 (mtime) はサーバのローカルタイムで表示する。`docs/` / `.team/artifacts/` / `.team/output/` の許可 3 ディレクトリ・path traversal 拒否などのセキュリティ境界は維持。詳細は `docs/spec/12-web-dashboard.md`
